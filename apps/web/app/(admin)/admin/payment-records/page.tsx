    ...
    <div style={{ padding: '0 10px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '40px', marginBottom: '30px' }}>
        ...
        <div style={{ display: 'flex', gap: '15px' }}>
          ...
          <button className={styles.btnOutline} style={{ display: 'flex', alignItems: 'center' }} onClick={() => setShowAuditModal(true)}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{marginRight:'8px'}}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" x2="8" y1="13" y2="13"/><line x1="16" x2="8" y1="17" y2="17"/><polyline points="10 9 9 9 8 9"/></svg> Audit Log
          </button>
      ...
      {showAuditModal && (
        <div className={styles.modalOverlay} onClick={e => { if (e.target === e.currentTarget) setShowAuditModal(false); }}>
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              <h3>Audit Log</h3>
              <span className={styles.modalClose} onClick={() => setShowAuditModal(false)}>&times;</span>
            </div>
            <div className={styles.modalBody} style={{ padding: '0' }}>
              {auditLogs.map((log, index) => (
                <div key={index} className={styles.auditItem}>
                  <span className={styles.auditTime}>{new Date(log.ts).toLocaleString()}</span>
            ...
            <div className={styles.modalFooter}>
              <button className={styles.btnOutline} onClick={() => setShowAuditModal(false)}>Close</button>
            </div>
          </div>
        </div>
      )}
